...
Он может опираться на поиск регулярками в текста модуля и/или анализировать элементы AST-дерева, 
...
dict(
    level='F', 
    patten='''kwargs<~ROW_N>1={'<~ID0>': <~GRP_NM>key<~GRP_OP><~TIE><~GRP_CL>}''',
    note='Рекомендую в строке <~ROW_N>1<~ROW_N> заменить {{patten_rt.g}} на args=[{{patten_rt.g.key}}].',
)
```
...
- [Свободно](#free-sugar) заменяемые/дополняемые. В умолчательный набор входят:  
    `‡‡` `□` `■` `○` `▪` `▫` `†`.  
...
- Замены "свободных" выполняются раньше замен "зашитых".
- В результате замены "свободных" могут появиться новые сахара, в том числе зоны изоляции.
...
dict(level='E', wo_strip='T', repeats='all',
    patten='<~ROW_N>1\\\n',
    note=['Так низяяяяя! В скобки их!', 'Смотри коммент к строке <~ROW_N>1<~ROW_N>.',],
),
```
*Пояснения.*  
– `wo_strip='T'` - не удалять пробельные символы из `patten`.  
– `repeats='all'` - комментировать все случаи.  
– `↓1` `<~ROW_N>1` - запомнить номер строки с первым найденным случаем.  
– `↓1↓`  `<~ROW_N>1<~ROW_N>` - вставить номер запомненной строки.  
...
class <~FILLER>
    ...
    def <~ROW_N>2<~FILLER>
    <~GAP>
    class Meta<~ROW_N>1<~FILLER>
''',], 
    note=['Не ложьте Мета за методы (см с.<~ROW_N>2<~ROW_N>)!', ' См выше (строка <~ROW_N>1<~ROW_N>).'],
),
dict( 
    patten=['''
class <~FILLER>
    ...
    <~ID0> = <~ROW_N>1<~FILLER>
''',], 
    note=['Не ложьте Мета перед полями!', ' См выше (строка <~ROW_N>1<~ROW_N>).'],
),
dict( 
    patten=['''
class <~FILLER>
    ...
    <~ID0> = <~ROW_N>1<~FILLER>
''',], 
    note=['Не класьте поля за методы (см с.<~ROW_N>2<~ROW_N>)!', ' См выше (строка <~ROW_N>1<~ROW_N>).'],
),
```
*Пояснения.*  
...
``` 
*Пояснения.*  
- `xpath:` - указывает, что шаблон - xpath-запрос к AST-XML-дереву.  
...
dict(level='E', note='Когда ж локализация у поля будет?',
    patten=[
      'xpath://Assign[targets/Name[@id="author"]]',
      'xpath://Assign[targets/Name[@id="author"]]'
...
или  
    xpath-выражением для AST-дерева (если начинается на `xa:`).  
...
- `≤имя≡сахар≥` <a name=frag-name></a>  
`<~GRP_NM>имя<~GRP_OP>сахар<~GRP_CL>`  
