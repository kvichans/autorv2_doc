...
1. На месте сахара может стать появится примитивная регулярка:  
...
1. Вставить сахар в шаблон можно несколькими способами:
...
1. Чтобы совместить "плюсы" от всех за и против, АР предоставляет возможность внутри 
...
```
Тут применены  
    `↓1` - **номер строки** (будет запоминаться в АР)  
...
dict(
    level='F', 
    patten='''kwargs~|ROW_N|1={'~|ID|': ~|GRP_BG|~|TIE|~|GRP_MD|key~|GRP_EN|}''',
...
– `‡‡` - идентификатор, т.е. словов из буквы-цифры-_-точка. `‡‡ = ` ловит имя поля.  
– `↓1` - запомнить номер строки с первым найденным случаем.  
– `↓1↓` - вставить номер запомненной строки.  
**Реакции**:  
...
```
*Пояснения.*  
– `‡‡` - идентификатор, т.е. словов из буквы-цифры-_-точка.  
