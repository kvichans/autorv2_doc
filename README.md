...
<details><summary>Другие заготовки</summary>

### Запрет на применение \ для продолжений кода <a name=ready_continue></a>
...
- `{{MON_ITER}}` - **номер итерации** (добавит [Монитор](#monitor), если он вызывает АР).
...
"личные" настройки (из `autorv2.cfg.py` или своего) и/или "умолчательные" из модуля `def_opts.py`.  
   ...
       `py10 autorv2.py -o skills/snk/skills/1=MY.JSON домашка.zip`  
...
- **Писк** после предыдущего шага.  
Полезен, так как АР2 будет работать "за кадром" неопределенно долго.  
Писком Монитор оповещает "*готово, можно грузить в Ревизор*"
- **Распаковка** zip-а.  
...
В любом элементе списка можно указывать подстановки `{{WRK}}` и `{{PRV}}`.  
