...
class Post''',
    ...
    frml='not {{pttn_a._0}} and ({{pttn_a._1}} or {{pttn_a._2}})', 
    posi='max({{pttn_a._1.e}}, {{pttn_a._2.e}})',
),
``` 
*Пояснения.*  
– `■` - любые символы в пределах этой же строки.  
– `¬°` - любое количество (в тч нулевое) строк с таким же отступом (тут - 4/8 пробелов).  
– `frml='...'` - текст формулы, которая после eval() скажет "успешен ли поиск".  
– `posi='...'` - текст формулы, которая после eval() скажет "номер строки, куда коммент вставлять".  
– `{{pttn_a._0}}` инфа про результат поиска первым шаблоном из списка.  
– Вычисление `not {{pttn_a._0}}` будет давать Да, если поиск провалился (инфа пустая).  
– `{{pttn_a._1.e}}` - номер последней строки для найденного фрагмента (тут второго).  
– Вычисление `max({{pttn_a._1.e}}, {{pttn_a._2.e}})` даст нижнюю строку от одного из найденных фрагментов.
...
class Comment''',
    ...
    note='Когда ж локализации-то будут?',
    frml='not {{pttn_a.clss_good}} and ({{pttn_a.clss_meta}} or {{pttn_a.clss_only}})', 
    posi='max({{pttn_a.clss_meta.e}}, {{pttn_a.clss_only.e}})',
),
```
*Пояснения.*  
– `{{pttn_a.clss_good}}` инфа про результат поиска clss_good:шаблоном.  
– Вычисление `not {{pttn_a.clss_good}}` будет давать Да, если поиск провалился (инфа пустая).  
– `{{pttn_a.clss_meta.e}}` номер последней строки для найденного фрагмента (тут от clss_meta:шаблона).  
– Вычисление `max({{pttn_a.clss_meta.e}}, {{pttn_a.clss_only.e}})` даст нижнюю строку от одного из найденных фрагментов.
...
class ┌‡‡┬name_clss_only┐''',
    ...
    note='Когда ж локализации-то будут?',
    frml='"{{pttn_a.g.name_clss_good}}"=="{{pttn_a.g.name_clss_meta}}"=="{{pttn_a.g.name_clss_only}}" and '
         'not {{pttn_a.clss_good}} and ({{pttn_a.clss_meta}} or {{pttn_a.clss_only}})', 
    posi='max({{pttn_a.clss_meta.e}}, {{pttn_a.clss_only.e}})',
),
```
*Пояснения.*  
...
– `"{{pttn_a.g.name_clss_good}}"` - подстановка значения переменной `name_clss_good`.  
