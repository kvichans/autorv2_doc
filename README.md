...
<details><summary>Этот же АР-словарь c **сахар-символами**</summary>  
...
есть полуфабрикаты для ревью Змейки.
...
dict(level='E', wo_strip='T', repeats='all',
    pattern='↓1\\\n',
    note=['Так низяяяяя! В скобки их!', 'Смотри коммент к строке {{_.↓1}}.',],
),
```
...
dict(level='E', wo_strip='T', repeats='all',
    pattern='<~ROW>1\\\n',
    note=['Так низяяяяя! В скобки их!', 'Смотри коммент к строке {{_.<~ROW>1}}.',],
),
```
...
*Пояснения.*  
- `wo_strip='T'` - не удалять "концы строк" из `pattern`.  
...
После поиска порождает runtime-[след](#pattern_rt) `hits`.  
...
    `'top'` и `'btm'` дадут -1, `'txt'` и `'txt_имя'` дадут пустую строку.  
...
dict(...,pattern='+t:def {{hits.nm}}{{hits.nm}}(',...),  # ищем def lalalala( (подходит для поисков в питоне)
dict(...,pattern='+t<<>>:<br><<hits.nm>>',...),          # ищем <br>lala (подходит для поисков в шаблоне)
dict(...,pattern='+te:"{{hits.nm}}"[:2]',...),           # ищем la
dict(...,formula='hits.nm[:2]=="ok"',...),               # проверяем "la"=="ok"
```
...
**Подстановка из еспансо по триггеру**  
Если в настройках еспансо есть указанный триггер, то произойдет подмена.  
...
- Внутри одного АР-словаря для `formula`, `position` и остальных значений с [подстановками](#subs-mod) доступны все [ключи](#rv-dict), как контекстные переменные с такими же именами:
   - `{{level}}`, `{{pattern}}`, `{{note}}` и т.д., в том числе [rt-следы](#rt-keys) `{{hits}}`.
...
## Шпаргалка "минимальных набор"<a name=min_brief></a>  

**АР-словарь**.

| Ключ<br>(!)=обязательный | Варианты значений  | Назначение               | Комментарий/Примеры   |
| --                    | --                    | --                       | --                    |
| [level](#levl) (!)    | `'E'`/`'F'`/`'G'`     | Ошибка/Совет/Похвала     |                       | 
| [pattern](#pttn) (!)  | `'...'`               | Что искать               | Внутри `note` как `{{hits._.txt}}`, <br>внутри `formula`/`position` как `hits._.top` | 
|                       | `['...', ...]`        |                          | Внутри `note` как `{{hits._0.txt}}`, <br>внутри `formula`/`position` как `hits._0.btm` |
|                       | `{'p1':'...'}`        |                          | `{{hits.p1.txt}}` и `hits.p1.top`         | 
...
| `‡`                | Минимальный "заполнитель между" <br>из любых (≥0) символов   | Стяжка |
...
| `≤name≡XX≥`        | Именованная группа | `def ≤fun≡‡‡≥(‡)` <br> Вставляется в `note` как <br>     `{{hits._.txt_fun}}` <br> Вставляется в `formula`/`position` как <br>     `hits._.top_fun` |
...
| `╔XX╗`             | Лишний текст **между** найденными | `походит ╔без этой части╗ фраза` |
|                    | **Изоляторы**      | |
...
1. Сахара
   ...
       `!<` `¦` `>!` стали `►` `▲` `◄`,  
       `╠` `╣` стали `╠` `╠`,  
       Внутри `note` вместо `↓1` применяется `{{_.↓1}}`.  
   - Удалены синонимы:  
       из `▪` и `¶±` остался только `▪`,  
       из `□` и `▬` остался только `□`,  
       из `▫` и `…` остался только `▫`,  
...
В АР2 это всегда регулярка на пути файлов.  
...
А в АР2 он ожидается в подсловаре  
...
- Создать "рыбу" для нескольких домашек: поиски по чекам без осмысленных комментариев.
...
- [cssselect](https://github.com/scrapy/cssselect) для работы с CSS3 selectors.
...
- [espanso.org](https://espanso.org/) автозамены текста.



# Благодарности
- Денис Унтевский. За соавторство.
- Игорь Шкода, Виталий Чернышов. За плодотворные обсуждения ТЗ.
- Артем Коломацкий. За полезную критику и за идеи для работы с AST-деревом.
- Максим Любиев. За идею с веб-сервером.
- Алексей Потапов. За фронт для Фласк-сервера. 
