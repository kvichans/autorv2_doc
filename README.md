...
class Post''',
    ...
    note='''Не хватает локализации модели.''',
...
class Comment''',
    ...
    note='''Не хватает локализации модели.''',
    frml='not {{pttn_a.clss_meta_vrb}} and ({{pttn_a.clss_meta}} or {{pttn_a.clss}})', 
    posi='max({{pttn_a.clss_meta.e}}, {{pttn_a.clss.e}})',
),
```
*Пояснения.*  
– `■` - любые символы в пределах этой же строки.  
...
– `{{pttn_a.clss_meta_vrb}}` инфа про результат поиска шаблоном по ключу `clss_meta_vrb`.  
Вычисление `not {{pttn_a.clss_meta_vrb}}` будет давать Да, если поиск провалился (инфа пустая).  
– `{{pttn_a._1.e}}` и `{{pttn_a.clss_meta.e}}` числа - номера последней строки для найденных фрагментов.  
