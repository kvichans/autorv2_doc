...
    давать подфрагменту имя `key` (по `┌┬key┐`).  
...
Например, можно создать файл `skills/skill_of_python.py` с таким наполнением  
   ...
   ```
1. Указать в настройка АР2 этот файл для проверяемой домашки.  
...
1. Указать эти знания в настройках для каждой конкретной домашки. Например, для Бота
   ```python
       ...
       skills=d(
           bot=d(
...
При доработках настроек АР перезапуск нужен.
1. Теперь можно сохранять из Ревизора zip-файл с домашкой в  
    `папка-всех-студентов` (или в `папка-всех-студентов/студент`).  
Монитор не создает подпапку `студент` - это ручная работа для ревьювера.  
Монитор  
    отследит появление zip,  
    запустит АР2,  
Для ревьювера это будет выглядет, как **zip-файл с комментариями появится сам**.  
Достаточно будет его подхватывать в Ревизоре.  
1. Если Монитор настроен с запуском дифф-тула, то после сохранения zip в папку 
    `папка-всех-студентов/студент`  
будет создаваться новая подпапка с исходниками домашки - имя будет дополняться номером итерации.  
Например, для Бота это будут папки  
    `homework_bot-master_1`,  
    `homework_bot-master_2` и тд.  
...
dict(levl='E', wo_strip='T', rpts='all'
    pttn='\\\n', 
    note=['Так низяяяяя! Применяйте заключение в скобки.','Смотри выше.',],
),
```
...
(*В разработке*)
### Неудачный порядок поля/Meta/методы в модели
(*В разработке*)
...
(*В разработке*)
### Ошибочное применение logger.error() и logging.error()
(*В разработке*)
...
Например, их можно указать на уровне [файла](#rv-file) с этим словарем или в опциях [запуска](#opts).  
...
- `pttn` (есть runtime-[след](#pttn_a) `pttn_a`) <a name=pttn></a>  
Один или несколько (списком или словарем) шаблонов для поиска.  
В случае успешного поиска (успех может уточняться в ожидаемом `frml`)  
формируется номер строки (расчет может уточнять в ожидаемом `posi`) для вставки комментария.  
- `note` (есть runtime-[след](#note_a) `note_a`) <a name=note></a>  
Один или несколько (списком или словарем) комментариев.  
Для первого найденного случая применяется первый комментарий, для второго - второй и т.д.  
Если потребуется больше применений (если `rpts` задан как `'all'`), то будет применяться последний.  
...
- `frml` <a name=frml></a>  
    Умолчание: `''` - будет расчет `any(pttn_a.values())`, то есть "есть хоть одна находка".  
...
- `posi` <a name=posi></a>  
    Умолчание: `''` - будет расчет `max(p.e for p in pttn_a.values())`, 
то есть "самая нижняя строка у найденных фрагментов".  
...
- `pttn_a` (словарь словарей) <a name=pttn_a></a>  
...
Любое значение (в том числе из списка/словаря) у обязательных/ожидаемых ключей может 
начинаться с модификаторов-указаний  
   - `+t:` `+t{{{}}}:`  
Нужны подстановки вида `{{имя}}`, где "имя" указывает на контектную переменную.
   - `+te:` `'+te<<>>:`  
После подстановок требуется `eval()`.  

Для значений у `frml` и `posi` эти модификаторы избыточны, так как применятся по-умолчанию.  

Между `+t`/`+te` и `:` могут указываться явные "парные скобки" для выделения подставляемого значения.  
Умолчательные "парные скобки" `{{}}`.
Пример.  
```python
dict(...,pttn='lala',name='nm',...),                  # ищем lala
dict(...,pttn='+t:def {{nm.pttn}}{{nm.pttn}}(',...),  # ищем lalalala в питоне
dict(...,pttn='+t<<>>:{{it}} <<nm.pttn>> ',...),      # ищем lala в шаблоне
...
- `¬°` - **на том же уровне сдвига** <a name=same-gap></a>  
