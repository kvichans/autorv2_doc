...
dict(
    ...
    note='Рекомендую в строке ↓1↓ заменить {{pattern_rt.g}} на args=[{{pattern_rt.g.key}}].',
)
```
Тут применены  
...
dict(
    ...
    note='Рекомендую в строке <~ROW>1<~ROW> заменить {{pattern_rt.g}} на args=[{{pattern_rt.g.key}}].',
)
```
...
**Пример**.  
Добавление плагинного сахара.  
В файле `autorv2_plug_sugars.py` нужно создать/импортировать класс вида
```python
class OneSeven(PlugSugar):
    SIGN  = '⅐'
    NAME  = '<~1_7>'
    def replace(self, pttn :str) ->str:
      pttn = pttn.replace(self.SIGN, '1/7')
      pttn = pttn.replace(self.NAME, '1/7')
      return pttn
...
```
*Пояснения.*  
– `wo_strip='T'` - не удалять пробельные символы из `pattern`.  
...
dict(level='E', note='Когда ж локализации у моделей будут?',
    ...
    formula='not {{pattern_rt._0}} and ({{pattern_rt._1}} or {{pattern_rt._2}})', 
    position='max({{pattern_rt._1.top}}, {{pattern_rt._1.top}})',
),
``` 
*Пояснения.*  
...
- `{{pattern_rt._0}}` набор найденных фрагментов от первого шаблона.  
Вычисление  
    `not {{pattern_rt._0}}`  
будет давать Да, если поиск провалился (фрагменты не найдены).  
Вычисление  
    `({{pattern_rt._1}} or {{pattern_rt._2}})`  
будет давать Да, если успешен хоть один из поисков по второму или третьему шаблону.  
- `{{pattern_rt._1.top}}` - номер верхней строки у фрагмента найденного вторым шаблоном.  
...
class Comment''',
    ...
    formula='not {{pattern_rt.good}} and ({{pattern_rt.only_meta}} or {{pattern_rt.only_clss}})', 
    position='max({{pattern_rt.only_meta.btm}}, {{pattern_rt.only_clss.btm}})',
),
```
*Пояснения.*  
...
- `formula='...'` - текст формулы, которая после eval() скажет "успешен ли поиск".  
- `position='...'` - текст формулы, которая после eval() скажет "номер строки, куда коммент вставлять".  
- `{{pattern_rt.good}}` инфа про результат поиска шаблоном `good=...`.  
- Вычисление  
    `not {{pattern_rt.good}}`  
будет давать Да, если поиск провалился (инфа пустая).  
- Вычисление  
    `({{pattern_rt.only_meta}} or {{pattern_rt.only_clss}})`  
будет давать Да, если если найден класс с Мета или только сам класс `Comment`.  
- `{{pattern_rt.only_meta.btm}}` номер последней строки для найденного фрагмента (тут от шаблона `only_meta=...`).  
- Так как `.btm` для ненайденных возвращает -1, вычисление  
    `max({{pattern_rt.only_meta.btm}}, {{pattern_rt.only_clss.btm}})`  
...
dict(level='E', note='Когда ж локализация у поля будет?',
    ...
    formula=' {{pattern_rt._0}} and not {{pattern_rt._1}}', 
    position='{{pattern_rt._0.top}}',
),
``` 
*Пояснения.*  
- Шаблоны начинаются на `/` - это XPath-запросы к AST-XML-дереву.  
...
- `position='...'` - текст формулы, которая после eval() скажет "номер строки, куда коммент вставлять".  
- `{{pattern_rt._0}}` - набор найденных фрагментов от первого шаблона.  
Вычисление `not {{pattern_rt._1}}` будет давать Да, если поиск провалился - `verbose_name` не найден для второго шаблона.  
- `{{pattern_rt._0.top}}` - номер верхней строки у фрагмента найденного первым шаблоном.  
...
dict(level='E', note='Когда ж локализация у поля будет?',
    ...
    formula='not {{pattern_rt.good}} and {{pattern_rt.bad}})', 
    position='{{pattern_rt.bad.btm}}',
),
```
...
- `{{pattern_rt.good}}` - инфа про результат поиска шаблоном `good=...`.  
- Вычисление `not {{pattern_rt.good}}` будет давать Да, если поиск провалился (инфа пустая).  
- `{{pattern_rt.bad.btm}}` номер последней строки для найденного фрагмента (тут от шаблона `bad=...`).  
...
dict(level='E', note='Когда ж локализация у поля будет?',
    pattern=dict(
        ...
        good='{{pattern_rt.g.name}} = ForeignKey(‡verbose_name‡)',
        bad='{{pattern_rt.g.name}} = ForeignKey(╔verbose_name╗)',
    ),
    formula='not {{pattern_rt.good}} and {{pattern_rt.bad}})', 
    position='{{pattern_rt.bad.btm}}',
),
...
- `{{pattern_rt.g.name}}` - подстановка ранее найденного фрагмента.
...
После поиска порождает runtime-[след](#pattern_rt) `pattern_rt`.  
- `note` <a name=note></a>  
    строка|список-строк|словарь-со-строковыми-значениями  
...
Умолчание - "есть хоть одна находка" - по формуле `any(pattern_rt.values())`.  
Например, при настройке  
    `formula='{{pattern_rt._0}} and {{pattern_rt._2}}'`  
...
Умолчание - "самая нижняя строка у найденных фрагментов" - по формуле `max(p.e for p in pattern_rt.values())`.  
Например,  
    `position='min({{pattern_rt._0.top}}, {{pattern_rt._1.btm}}, -1)'`.  
- `wo_strip` <a name=wo_strip></a>  
    Умолчание: `''` - выполнять strip-ы.  
Флаг для **пропуска** `.strip()` для всех значений у `pattern` и `note`.  
...
- `pattern_rt`/`pttn_rt` <a name=pttn_rt></a>  
    словарь словарей  
...
    `{'_0':{инфо},'_1':{},}`  
Если `pattern` уже был словарем, то ключи сохранятся.  
...
    список словарей  
Список всех успешных **применений**, содержащий словари с ключами `'pattern_rt'` и `'note_rt'`.  
Набор `usages` не пуст, если было хоть одно успешное применение.  
В этом случае в  
    `usages.0.pattern_rt` и `pattern_rt` совпадают,  
...
Пример.  
```python
dict(...,pattern='lala',name='nm',...),                  # ищем lala
dict(...,pattern='+t:def {{nm.pattern}}{{nm.pattern}}(',...),  # ищем lalalala в питоне
dict(...,pattern='+t<<>>:<br>{{it}} <<nm.pattern>> ',...),  # ищем lala в шаблоне
dict(...,pattern='+te:"{{nm.pattern}}"[:2]',...),           # ищем la
```
...
- Поиск `om` только внутри комментариев и док-строк.  
   ```python
   dict(...,pattern='+ast:om',formula='"{{pattern_rt.ast}}" in "com doc"',...),
   ```
...
    `↓1↓` - от первого элемента (из списка или словаря) `pattern`.  
    `↓_0.1↓` - от первого элемента, если `pattern` задан списком.  
    `↓имя.1↓` - от элемента `pattern` с ключом `имя`.  
...
Назначенное имя можно будет применять в через `pattern_rt.g.имя` в ключах АР-словаря. 
...
- Внутри одного АР-словаря для `formula`, `position` и остальных значений с [подстановками](#subs-mod) доступны все [ключи](#rv-dict), как контекстные переменные с такими же именами:
   - `{{level}}`, `{{pattern}}`, `{{note}}` и т.д., в том числе [rt-следы](#rt-keys)  
   `{{pattern_rt}}`, `{{note_rt}}`, `{{usages}}`.
   - Если значение список, то доступ по именам `{{pattern._0}}`, `{{pattern._1}}` и т.д.  
   При этом совпадают `{{pattern}}` и `{{pattern._0}}`.
   - Если значение словарь, то доступ по его ключам `{{pattern.ключ}}`, `{{pattern.еще}}` и т.д.  
   При этом совпадают `{{pattern}}` и `{{pattern.первый-ключ}}`.
- От предыдущего (см выше) АР-словаря с заданым ключом `name='имя'` доступны все эти же данные с префиксом `имя.`  
   - `{{имя.level}}`, `{{имя.pattern}}`, `{{имя.note}}` и т.д.
...
# Запуск теста
mt  = re.search(pattern, src)
if not mt:return print('fail')
print(mt.groupdict())
```

### Рекомендуемый способ применения
Чтобы не заморачиваться с расчетом пути до нужного pattern в полном json-дереве,  
можно завести один пи-исходник `ds.py` с тривиальным наполненем  
    ["""шаблон"""]  
копировать в него нужный для отладки pattern и запускать конвертацию всегда одинаково  
...
- Добавить применение захваченных групп внутри набора шаблонов одного АР-словаря.
